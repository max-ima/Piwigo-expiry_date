...
<span id="expd_options" style="{if !isset($filter.prefilter) or $filter.prefilter ne "expiry_date"}display:none{/if}">
  ...
  <label class=""><input type="radio" name="filter_expd" value="30" {if isset($filter.expiry_date_option) && 30 == $filter.expiry_date_option}checked{/if}> {"less than %s days"|translate:30}</label>
</span>